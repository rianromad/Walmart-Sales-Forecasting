...
#melihat tipe data masing-masing atribut (train)
train.info()

#melihat tipe data masing-masing atribut (stores)
stores.info()

#melihat tipe data masing-masing atribut (features)
features.info()

#cek missing value data features
features.isnull().sum()

#statistika deskriptif
train.merge(stores, how='left').merge(features, how='left').describe().T

#sebaran penjualan mingguan
fig, ax = plt.subplots(figsize=(20,5))
sns.histplot(train['Weekly_Sales'], bins=30, kde=True, ax=ax)
...
#mengubah tipe data pada atribut tanggal menjadi datetime
df_train['Date'] = pd.to_datetime(df_train['Date'])

#cek kembali tipe data Date
df_train.info()

#Transformasi atribut IsHoliday dan Type
...
date(2012,10,26)-date(2012,1,6)

#evaluasi model dilakukan dengan data dari tanggal 11 Februari 2011
cutoffs = pd.to_datetime(['2012-01-6'])
df_cv = cross_validation(prophet_m, cutoffs=cutoffs, horizon='294 days')

#evaluasi model FB Prophet dari tanggal 13 Januari 2013 dst
...
# plot hasil prediksi
plot_plotly(prophet_m, prophet_forecast)
